...
def test_ffi_c_kernel_direct_execution():
    """Verify raw FFI call into libinvolute.so via ctypes."""
    raw_word = 0xAAAAAAAAAAAAAAAA
    boundary_mask = 0xFFFFFFFFFFFFFFFF

    ground_truth, is_valid = run_involute_eval(raw_word, boundary_mask)
...
def test_c_kernel_boundary_breach():
    ...
    # Active bit outside mask forces invariant collapse to zero
    raw_word = 0x1
    boundary_mask = 0x0
